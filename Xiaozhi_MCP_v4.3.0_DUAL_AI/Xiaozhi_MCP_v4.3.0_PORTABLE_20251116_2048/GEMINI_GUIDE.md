...
|-------|--------|------------|----------|
| `gemini-2.0-flash-exp` | ⚡⚡⚡ Rất nhanh | ⭐⭐⭐ Tốt | **Mặc định** - Cân bằng tốc độ và chất lượng |
| `gemini-1.5-pro` | ⚡ Chậm | ⭐⭐⭐⭐⭐ Xuất sắc | Tasks phức tạp, phân tích sâu |
| `gemini-1.5-flash` | ⚡⚡⚡ Rất nhanh | ⭐⭐⭐ Tốt | Chat đơn giản, hỏi đáp nhanh |

**Khuyến nghị:**
- Dùng `gemini-2.0-flash-exp` cho hầu hết các trường hợp (mặc định)
- Dùng `gemini-1.5-pro` khi cần câu trả lời chi tiết, phức tạp
- Dùng `gemini-1.5-flash` khi cần tốc độ tối đa
...
```json
{
  ...
  "message": "✅ Gemini đã trả lời (model: gemini-2.0-flash-exp)"
}
```
...
1. Đổi sang model nhanh hơn: `gemini-1.5-flash`
2. Rút ngắn prompt
