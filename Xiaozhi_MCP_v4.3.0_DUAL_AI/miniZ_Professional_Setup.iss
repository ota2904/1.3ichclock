...
#define MyAppDescription "MCP Server with AI Integration - Full Persistence Fix"
...
VersionInfoTextVersion=4.3.7 Professional (Full Persistence Fix)
...
OutputBaseFilename=miniZ_MCP_v4.3.7_Professional_Setup
...
Name: "autostart"; Description: "🚀 Tự động khởi động cùng Windows (Khuyến nghị)"; GroupDescription: "Tùy chọn nâng cao:"

[Files]
; ========== Core Application - EXE ==========
Source: "dist\miniZ_MCP_Professional.exe"; DestDir: "{app}"; Flags: ignoreversion

; ========== Core Application - Python (Backup) ==========
...
Source: "CHECK_AUTOSTART.bat"; DestDir: "{app}"; Flags: ignoreversion
Source: "INSTALL.bat"; DestDir: "{app}"; Flags: ignoreversion
Source: "FIRST_RUN.bat"; DestDir: "{app}"; Flags: ignoreversion
Source: "AUTO_INSTALL_DEPENDENCIES.bat"; DestDir: "{app}"; Flags: ignoreversion

; ========== Configuration ==========
Source: "requirements.txt"; DestDir: "{app}"; Flags: ignoreversion
Source: "xiaozhi_endpoints.json"; DestDir: "{app}"; Flags: ignoreversion; BeforeInstall: BackupConfigFile
...
Name: "{group}\Kiểm tra Auto-Start"; Filename: "{app}\CHECK_AUTOSTART.bat"; WorkingDir: "{app}"; Comment: "Kiểm tra tự động khởi động"; Tasks: startmenu
...
Filename: "{app}\AUTO_INSTALL_DEPENDENCIES.bat"; Description: "📦 Cài đặt Python dependencies (BẮT BUỘC - chạy lần đầu)"; Flags: postinstall skipifsilent runhidden waituntilterminated; StatusMsg: "Đang cài đặt thư viện Python..."; Check: not FileExists(ExpandConstant('{app}\.dependencies_installed'))
...
Filename: "{app}\FIRST_RUN.bat"; Description: "🚀 Khởi động {#MyAppName} ngay"; Flags: postinstall skipifsilent nowait; WorkingDir: "{app}"
...
begin
  ...
  // Check length (19 chars with dashes or 16 without)
  if (Length(Key) <> 19) and (Length(Key) <> 16) then
    Exit;
  ...
  // Valid format: XXXX-XXXX-XXXX-XXXX (3 dashes) or no dashes
  if (dashCount = 3) or (dashCount = 0) then
    Result := True;
end;
  ...
  begin
    ...
    begin
      MsgBox('❌ License Key không hợp lệ!' + #13#10#13#10 + 
             ...
             'XXXX-XXXX-XXXX-XXXX' + #13#10#13#10 +
...
// Backup config procedure - called before overwriting
procedure BackupConfigFile();
var
  AppPath, ConfigFile, BackupFile: String;
begin
  AppPath := ExpandConstant('{app}');
  ConfigFile := AppPath + '\xiaozhi_endpoints.json';
  
  if FileExists(ConfigFile) then
  begin
    BackupFile := AppPath + '\xiaozhi_endpoints.json.backup.' + GetDateTimeString('yyyymmdd_hhnnss', #0, #0);
    if CopyFile(ConfigFile, BackupFile, False) then
      Log('✅ Backed up old config to: ' + BackupFile)
    else
